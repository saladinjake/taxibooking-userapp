...
class Map extends Component {
  constructor(props) {
    ...
    this.state = {
      center: { lat: 28.589664, lng: 77.05742 },
      vehicles: [],
      id: -1,
      colors: [],
      card: 0,
    ...
    return (
      <div className="row">
        <div className="col-md-8 mapcont">
          <div ref="map" className="map" style={{ display: 'block' }}></div>
          <br />
        </div>
        <div className="col-md-4">
          ...
          <div className="alert alert-secondary" role="alert">
            *Add a vehicle navigation on the map by submitting the source and destination addresses.
            <br />
            *Click on the Vehilce Buttons for information of that vehicle and to locate its current
            position on the map.
          </div>
          ...
          <div>
            {this.state.vehicles.map((item, index) => (
              <button
                ...
                onClick={event => this.panMap(item, index)}
              >
                Vehicle {index + 1}
              </button>
            ))}
          ...
          {this.state.vehicles.length !== 0 && (
            <div
              ...
              style={{ backgroundColor: this.state.colors[this.state.card] }}
            >
              <div className="card-header">Vehicle {this.state.card + 1}</div>
              <div className="card-body" style={{ color: 'white' }}>
                <h5 className="card-text">
                  <b>Origin:</b> {this.state.vehicles[this.state.card][0][0]}
                </h5>
                <h5 className="card-text">
                  <b>Destination:</b> {this.state.vehicles[this.state.card][1][0]}
                </h5>
              </div>
            </div>
          )}
