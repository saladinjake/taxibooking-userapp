        ...
        if(userEntity!=''){
			if(MethodType=='GET'){
	           logMessage =`Fetch Record by ${adminEntity.user.username} for the Api resource `
				...
				logMessage =`Post Record by ${adminEntity.user.username} operation regarding resource/entity for ${userEntity} on ${formatDate(new Date())} `

			}else if(MethodType=='PUT'){

				logMessage =`Edit Record by ${adminEntity.user.username} operation  on ${formatDate(new Date())} has a  labeled status of ${messageType} `

			}else if(MethodType=='DELETE'){
				logMessage =`Delete Record ${adminEntity.user.username} operation on ${formatDate(new Date())} has status ${messageType} .`
