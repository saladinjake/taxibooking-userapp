    ...
    .then(data => {
      ...
      if (data.status == 200 || data.status == 201 || data.status == 'ok') {
        AuditTrail.sendLogInfo(
          ...
          '201',
          'PUT',
          'user drive test updated .',
        );

        var notification = alertify.notify(
          'Successfully updated drive test ',
          'success',
          5,
          function() {
  ...
  fetch(url, {
    ...
    .then(data => {
      ...
      if (data.status == 201) {
        ...
        var notification = alertify.notify(
          'Successfully created drive test ',
          'success',
          5,
          function() {
            console.log('dismissed');
        ...
        AuditTrail.sendLogInfo(
          ...
          '400',
          'POST',
          'Drive test failed to create resource',
        );
        ...
        .then(data => {
          if (data) {
            ...
            AuditTrail.sendLogInfo(
              ...
              '200',
              'GET',
              'Users detailed was fetched successfully for ' + userRecord.firstname,
            );
          } else {
            var notification = alertify.notify(
              'Could not perform Update location operation',
              'error',
              5,
              function() {
                console.log('dismissed');
        ...
        .then(data => {
          if (data) {
            ...
            AuditTrail.sendLogInfo(
              ...
              '200',
              'GET',
              'User profile resource successful for ' + userRecord.username,
            );
            ...
            AuditTrail.sendLogInfo(
              ...
              '200',
              'GET',
              'User profile resource failed for' + userRecord.username,
            );
...
function updateUsersTestCenter(url, prePostData) {
  ...
  if (!test_center.length) {
    var notification = alertify.notify('test center field required.', 'error', 5, function() {
      console.log('dismissed');
    });
  ...
  if (!test_center_address.length) {
    var notification = alertify.notify('Address required.', 'error', 5, function() {
      console.log('dismissed');
    });
  ...
  fetch(url, {
    ...
    .then(data => {
      ...
      if (data.status == 200) {
        var notification = alertify.notify(
          'Successfully update of users test location',
          'success',
          5,
          function() {
            console.log('dismissed');
        ...
        AuditTrail.sendLogInfo(
          ...
          '201',
          'POST',
          'User test center location updated',
        );
      } else {
        AuditTrail.sendLogInfo(
          user,
          ...
          '200',
          'GET',
          'Test center location update for user failed',
        );

        var notification = alertify.notify(
          'Could not perform update operation for users test location.',
          'error',
          5,
          function() {
            console.log('dismissed');
...
window.genCert = o => {
  document.getElementById('certificate' + o.dataset.id).value =
    'CMT-DRIV-CERT-' + guidGenerator().substring(0, 20);
};
...
function getUserRights() {
  ...
  if (localStorage.getItem('userToken')) {
    const user = JSON.parse(localStorage.getItem('userToken'));
    let linkOfApi =
      baseUrl + '/profile-admin-rights/update/' + user.user._id + '/permission/' + user.user.roles;
    ...
    return fetch(linkOfApi, {
      ...
      .catch(e => {
        if (localStorage.getItem('previledges') == 'undefined') {
          var notification = alertify.notify(
            'Slow network....please reload the webpage or restart the service.',
            'error',
            5,
            function() {
              console.log('dismissed');
  ...
  fetch(postUrl, {
    ...
    .then(data => {
      if (data.status === 201) {
        ...
        AuditTrail.sendLogInfo(
          user,
          userPlanItineries.username,
          'BOOKINGS Module',
          'Success',
          '201',
          'POST',
          'User Bookings created for ' + userPlanItineries.username,
        );
        ...
        AuditTrail.sendLogInfo(
          ...
          '403',
          'POST',
          'Failed to create booking for ' + userPlanItineries.username,
        );
        ...
        var notification = alertify.notify('Error in posting data.', 'error', 5, function() {
          console.log('dismissed');
        });
      }
    })
    ...
    .then(data => {
      if (data.status === 201) {
        ...
        AuditTrail.sendLogInfo(
          ...
          '201',
          'POST',
          'Quotation created for ' + userqUOTA.username,
        );
      } else if (data.status === 401 || data.status === 403) {
        AuditTrail.sendLogInfo(
          user,
          ...
          '201',
          'POST',
          'Failed to create Quotation for ' + userqUOTA.username,
        );
        ...
        AuditTrail.sendLogInfo(
          ...
          '201',
          'POST',
          'Failed to create quotation ' + userqUOTA.username,
        );

        //MessageBoard.displayMsg(data.error);
        var notification = alertify.notify('Error in posting quotation.', 'error', 5, function() {
          console.log('dismissed');
        });
      }
    })
...
function BookingValidationFails(planItineries) {
  ...
  if (username.length <= 0) {
    validationFails = true;
    console.log('location err');
    var notification = alertify.notify('username is required', 'error', 5, function() {
      console.log('dismissed');
    });
  }
  ...
  if (phone_number.length <= 0) {
    validationFails = true;
    console.log('location err');
    var notification = alertify.notify('phone is required', 'error', 5, function() {
      console.log('dismissed');
    });
  }
  ...
  fetch(postUrl, {
    ...
    .then(data => {
      if (data.status === 201 || data.status === 200) {
        console.log(data);
        //MessageBoard.displayMsg('Form submitted succesfully');
        var notification = alertify.notify('Successful transaction.', 'success', 5, function() {
          console.log('dismissed');
        });
        ...
        AuditTrail.sendLogInfo(
          ...
          '201',
          'POST',
          'Created post notification for ' + prePostData.email,
        );
        ...
        AuditTrail.sendLogInfo(
          ...
          '400',
          'POST',
          'Post Notification failed for ' + prePostData.email,
        );
        ...
        AuditTrail.sendLogInfo(
          ...
          '400',
          'POST',
          'Failed  to create Notification for ' + prePostData.email,
        );
        //MessageBoard.displayMsg(data.error);
        var notification = alertify.notify(
          'Failed to send notfication to user',
          'error',
          5,
          function() {
            console.log('dismissed');
...
window.showModalVideo = function(el) {
  ...
  document.getElementById(el.dataset.id).addEventListener('click', function() {
    document.getElementById(
      'modvideo',
    ).innerHTML = `<video class="embed-responsive-item" id="demoVideo"    controls>
...
const addClick = () => {
  ...
  if (document.getElementById('car')) {
    ...
    document.getElementById('car').src =
      'https://Goom Logistics-bucket.s3.amazonaws.com/unnamed.jpg';
  }
  ...
  fetch(linkOfApi, {
    ...
    .then(data => {
      ...
      if (data.success === 'ok') {
        ...
        var notification = alertify.notify(
          'Successful updated of user verification ',
          'success',
          5,
          function() {
            console.log('dismissed');
...
window.addEventEarnings = o => {
  ...
  if (partnerEmail_x.options[partnerEmail_x.selectedIndex].text == '--select--') {
    var notification = alertify.notify('Select a valid email.', 'error', 5, function() {
      console.log('dismissed');
    });
  ...
  if (isNaN(amt)) {
    var notification = alertify.notify('Enter a valid amount.', 'error', 5, function() {
      console.log('dismissed');
    });
  ...
  fetch(linkOfApi, {
    ...
    .then(data => {
      if (data.status === 201) {
        // modal_view_id.style.display="none";
        var notification = alertify.notify(
          'Successfully created  partners earnings. Please wait...',
          'success',
          5,
          function() {
            console.log('dismissed');
        ...
        AuditTrail.sendLogInfo(
          ...
          '201',
          'POST',
          'Partner earnings created for ' + prePostData.username,
        );
        ...
        AuditTrail.sendLogInfo(
          ...
          '400',
          'POST',
          'Failed to create partner earnings for ' + prePostData.username,
        );
...
window.addEvent = o => {
  ...
  if (validResult) {
    fetch(linkOfApi, {
      ...
      .then(data => {
        if (data.status === 201) {
          modal_view_id.style.display = 'none';
          var notification = alertify.notify(
            'Successfully created  resource. Please wait...',
            'success',
            9,
            function() {
              console.log('dismissed');
          ...
          AuditTrail.sendLogInfo(
            user,
            prePostData.username,
            'USER ENTITY  MODULE(USER/DRIVER/ADMIN)',
            'Success',
            '201',
            'POST',
            'User MODULE(USER/DRIVER/ADMIN) created for ' + prePostData.username,
          );
          setTimeout(() => {
            window.location.reload();
          }, 3000);
          ...
          AuditTrail.sendLogInfo(
            ...
            '400',
            'POST',
            'USER ENTITY  MODULE(USER/DRIVER/ADMIN) Failed',
          );

          var notification = alertify.notify(
            'Could not perform add operation. probably a duplicate field error in the field phone number',
            'error',
            5,
            function() {
              console.log('dismissed');
    ...
    var notification = alertify.notify('unsuccessful add operation.', 'error', 5, function() {
      console.log('dismissed');
    });
  }
};
...
window.updateRecordItins = o => {
  ...
  if (document.getElementById('drive_option' + view_id).value == 'I would like to drive myself') {
    ...
    if (drivers.options[drivers.selectedIndex].text == '--Please select a driver--') {
      var notification = alertify.notify('Select a driver', 'error', 5, function() {
        console.log('dismissed');
      });
  ...
  fetch(linkOfApi, {
    ...
    .then(data => {
      ...
      if (data.status) {
        ...
        AuditTrail.sendLogInfo(
          user,
          prePostData.assigned_driver_name,
          'ITINERARY MODULE (DRIVER ASSIIGNMENT)',
          'Success',
          '201',
          'POST',
          'Itinerary Assignment created',
        );
      } else {
        AuditTrail.sendLogInfo(
          user,
          ...
          '400',
          'POST',
          'Itinerary failed to assign driver',
        );
...
window.updateRecordView = el => {
  ...
  if (addMethodTrigger == false) {
    document.getElementById('user-id').innerHTML =
      'CMT-USER-' + view_id.substring(-12, view_id.length);
  } else {
    document.getElementById('user-id').innerHTML = '';
  }
};
...
window.viewPlan = el => {
  ...
  document.getElementById('user-id').innerHTML =
    'CMT-PLAN-' + view_id.substring(-5, view_id.length);
...
window.updatePlanData = o => {
  ...
  if (validResult) {
    fetch(linkOfApi, {
      ...
      .then(data => {
        ...
        if (data.success === 'ok') {
          ...
          AuditTrail.sendLogInfo(
            user,
            prePostData.plan_name,
            'PLAN CATEGORY ',
            'Success',
            '201',
            'POST',
            'Plan category created for ' + prePostData.plan_name,
          );
          ...
          AuditTrail.sendLogInfo(
            ...
            '400',
            'POST',
            'Failed to create plan category',
          );

          var notification = alertify.notify(
            'Could not perform update operation. Ensure the plan selected is correct.',
            'error',
            5,
            function() {
              console.log('dismissed');
    ...
    var notification = alertify.notify('unsuccessful update operation', 'error', 5, function() {
      console.log('dismissed');
    });
  }
};
  ...
  fetch(linkOfApi, {
    ...
    .then(data => {
      ...
      if (data.success === 'ok') {
        ...
        var notification = alertify.notify('Successful updated ', 'success', 5, function() {
          console.log('dismissed');
        });
        window.location.reload();
        // ApiDeleteOneStatusRecord.redirect(recordOfType);
...
window.updateCarRecordTemplate = o => {
  ...
  if (status_x.options[status_x.selectedIndex].text == '--Select Status--') {
    var notification = alertify.notify('Status required', 'error', 5, function() {
      console.log('dismissed');
    });
    ...
    .then(data => {
      ...
      if (data.status == 200) {
        ...
        AuditTrail.sendLogInfo(
          ...
          '200',
          'PUT',
          'Car record  successfully updated for ' + prePostData.car_name,
        );
...
window.viewRecordTemplate = el => {
  ...
  if (document.getElementById('subject' + view_id)) {
    //tickets

    document.getElementById('date' + view_id).value = el.dataset.date;

    document.getElementById('user_id' + view_id).value = el.dataset.username;
    document.getElementById('ticket_id' + view_id).value =
      'CMT-RECORD-' + view_id.substring(-12, view_id.length);
  ...
  } else if (document.getElementById('plate_number' + view_id)) {
    //sos
    document.getElementById('date' + view_id).value = el.dataset.date;
  ...
  } else if (document.getElementById('admin-repairs-status' + el.dataset.id)) {
    // mechanic request
  ...
  document.getElementById('user-id').innerHTML =
    'CMT-RECORD-' + view_id.substring(-5, view_id.length);
...
window.addRecordEvent = o => {
  ...
  if (document.getElementById('category' + view_id)) {
    ...
    if (user_email == '--Select an email user--') {
      var notification = alertify.notify('Select an email', 'error', 5, function() {
        console.log('dismissed');
      });
      return false;
    }
  ...
  fetch(linkOfApi, {
    ...
    .then(data => {
      ...
      if (data.status === 201) {
        ...
        var notification = alertify.notify('Successful created.', 'success', 5, function() {
          console.log('dismissed');
        });
        ...
        AuditTrail.sendLogInfo(
          user,
          prePostData.username,
          'SUPPORT MANAGEMENT(TICKETS/SOS/FAQS)',
          'Success',
          '201',
          'POST',
          'Support ticket response successful',
        );

        if (document.getElementById('category' + view_id)) {
          let data_msg = 'Dear ' + user.user.username + ' ';
          data_msg +=
            ' A Ticket has been created  for you. Your TICKET ID:' +
            document.getElementById('ticket_id' + view_id).value +
            'on our platform  and is currently being reviewed by our support team. The status of this ticket is of now' +
            status;
          ...
          if (response) {
            let data_msg = 'Dear ' + user.user.username + ' ';
            data_msg +=
              ' Following with the reviews of the ticket created initially for the TICKET ID:' +
              document.getElementById('ticket_id' + view_id).value +
              'on our platform  and is currently being reviewed by our support team. The status of this ticket is of now' +
              status;
  ...
  fetch(linkOfApi, {
    ...
    .then(data => {
      ...
      if (data.status === 201 || data.status == 200) {
        ...
        var notification = alertify.notify('Successfully updated record', 'success', 5, function() {
          console.log('dismissed');
        });
        ...
        AuditTrail.sendLogInfo(
          ...
          '201',
          'PUT',
          'Ticket response was successful',
        );

        if (tickets == true) {
          let notification_template = `<div class="media-body">
		                                                    <h5 class="media-heading">Dear ${
                                                          user.username
                                                        }, your  ticket with the given id  CMT-RECORD-${view_id.substring(
            -12,
            view_id.length,
          )} has been placed.</h5>
		                                                    <p class="m-0">
          ...
          let linkOfApi2 =
            activeUrl + '/notification-update/' + document.getElementById('email' + view_id).value;
          fetch(linkOfApi2, {
            method: 'PUT',
            headers: {
              Accept: 'application/json',
...
window.addCarRecordEvent = o => {
  ...
  if (avatar.length <= 0) {
    ...
    var notification = alertify.notify('Upload an image for the car', 'error', 5, function() {
      console.log('dismissed');
    });
  ...
  fetch(linkOfApi, {
    ...
    .then(data => {
      if (data.status === 201) {
        ...
        modal_view_id[0].style.display = 'none';
        //document.getElementById("gtd").classList.remove("overlay")
        var notification = alertify.notify('Successfully created  car.', 'success', 5, function() {
          console.log('dismissed');
        });
        ...
        AuditTrail.sendLogInfo(
          ...
          '201',
          'POST',
          'Successfully created car ' + prePostData.car_name,
        );
      ...
      .then(data => {
        if (data.status === 201) {
          ...
          AuditTrail.sendLogInfo(
            ...
            '201',
            'PUT',
            'Plan created successfully for ' + prePostData.plan_name,
          );
  ...
  fetch(linkOfApi, {
    ...
    .then(data => {
      ...
      if (data.success === 'ok') {
        modal_view_id.style.display = 'none';
        var notification = alertify.notify(
          'Successfully updated partners earnings',
          'success',
          5,
          function() {
            console.log('dismissed');
        ...
        AuditTrail.sendLogInfo(
          user,
          prePostData.username,
          'USER MODULE (ADMIN/USER/DRIVER)',
          'Success',
          '201',
          'PUT',
          'Partner earnings updated for ' + prePostData.partnerEmail,
        );
        ...
        AuditTrail.sendLogInfo(
          ...
          '201',
          'PUT',
          'Partner earnings update failed',
        );
      ...
      .then(data => {
        ...
        if (data.success === 'ok') {
          ...
          AuditTrail.sendLogInfo(
            ...
            '201',
            'PUT',
            'Successfully updated' + prePostData.username + 'record',
          );
...
const deleteOperation = async o => {
  ...
  if (confirm) {
    ...
    var notification = alertify.notify('Delete Operation was canceled', 'warning', 5, function() {
      console.log('dismissed');
    });
    setTimeout(() => {
      window.location.reload();
...
window.deleteData = o => {
  ...
  if (o.dataset.delete_type) {
    linkOfApi =
      activeUrl + o.dataset.url + '/' + o.dataset.id + '/delete' + '/' + o.dataset.delete_type;
  } else {
  ...
  fetch(linkOfApi, {
    ...
    .then(data => {
      if (data.status === 202) {
        modal_view_id.style.display = 'none';
        //document.getElementById("gtd").classList.remove("overlay")
        var notification = alertify.notify('Successful delete operation', 'success', 5, function() {
          console.log('dismissed');
        });
      ...
      .then(data => {
        ...
        if (data.success === 'ok') {
          ...
          AuditTrail.sendLogInfo(
            ...
            '201',
            'PUT',
            'Profile update success for ' + prePostData.email,
          );
          ...
          var notification = alertify.notify(
            'Could not perform update profile operation',
            'error',
            5,
            function() {
              console.log('dismissed');
...
class ApiAdminBotService {
  ...
  static runDashboard(
    ...
    ticketList.map((item, i) => {
      ...
      template2 = `<tr>
                         <td><a onclick="viewPlan(this)" href="#" id="plancat${
                           item._id
                         }" data-id="${
        item._id
      }" data-url="/admin-ticket-detail" class=""><b>${formatDate(
      ...
      notice.map((item, i) => {
        if (item.type == 'payment' && item.status == 'new') {
          ...
          let markup = `   <div class="pull-left p-r-10" style="" id="${i}">
                                                       ...
                                                       <div onclick="updateNotificationStatus(this)" data-id="${
                                                         item._id
                                                       }" class="media-body">
                                                          ...
                                                          <hr/>
                                                          <p class="m-0" >
                                                              <small style="">${item.description.substring(
                                                                0,
                                                                150,
                                                              )}...<a id="plan-current-5e8904789ee841375ce073f3" onclick="" data-id="5e8904789ee841375ce073f3" href="./admin-bookings" class="table-action-btn btn-custom btn-purple"><i class="md md-chevron-right"></i></a></small>
                                                          </p>
                                                       </div><hr/>`;
  ...
  static runAdminPartnersEarnings(datas, partners, cars, previledges) {
    ...
    [...new Set(cars)].map((item, i) => {
      selectedCars += `<option data-id="${item.creator}" data-vehicleplateno="${item.plate_number ||
        item.plateNumber}" id="${item._id}"  data-vehiclename="${item.model || item.Model}">${item
        .car.car_name || item.Model}-${item._id}</option>`;

      carsR += `
              <li onclick="viewCarDetail(this)"  data-id="${
                item.creator
              }" data-plate_no="${item.plateNo ||
        item.plate_number}" data-desc="${item.description ||
        item.carDescription}" data-image="${item.images || item.imagePath}" data-name="${
        item.car.car_name
      }" data-carid="${item._id}" id="${
        item._id
      }"   class="classless"><input class="example" type="checkbox" id="cb${i + 1}" data-id="${
        item.creator
      }" />
                <label class="classless" for="cb${i +
                  1}"><img    style="height:80px; width:70px" src="${item.images}" /></label>
              </li>`;
    });
    ...
    datas.map((item, i) => {
      ...
      template2 += `<tr class="notification">
                    ...
                    <td class=""><span class="label ${className}">${item.paymentStatus ||
        ''}</span></td>

                    <td class=""><a onclick="updateRecordViewEarnings(this)" data-toggle="modal" data-bankaccountname="${item
                      .partnerBankAccount.bankAccountName || ''}" data-bankaccountnumber="${item
        .partnerBankAccount.bankAccountNumber || ''}" data-vehicleId="${item.vehicleId ||
        ''}"  data-vehicleplateno="${item.vehiclePlateNo || ''}"   data-id="${
        item._id
      }" data-paymentamount="${item.paymentAmount || ''}" data-partneremail="${item.partnerEmail ||
        ''}" data-partnerBankAccount="${item.partnerBankAccount.bankAccount ||
        ''}" data-PartnerEmail="${item.PartnerEmail ||
        ''}" data-paymentReference="${item.paymentReference || ''}"  data-paymentdate="${formatDate(
        new Date(item.paymentDate),
      ) || ''}" data-paymentid="${item.paymentId || ''}" data-partnerid="${item.partnerId ||
        ''}" data-vehiclename="${item.vehicleName ||
        ''}"  class="table-action-btn"><i class="md md-edit"></i></a>
                    <a onclick="deleteRecord(this)" data-id="${
                      item._id
                    }" data-url="/partners-earnings"  class="table-action-btn "><i class="md md-close"></i></a></td>
                </tr>`;
  ...
  static runAdminPartners(datas, previledges) {
    ...
    datas.map((item, i) => {
      ...
      if (item.avatar.length > 0) {
        avt = item.avatar.startsWith('https://')
          ? item.avatar
          : 'https://Goom Logistics-bucket.s3.amazonaws.com/' + item.avatar;
      } else {
      ...
      template2 += `<tr class="notification">
                    ...
                    <td class=""><a onclick="updateRecordView(this)"  data-bankaccount="${item.bankAccount ||
                      ''}"  data-bankaccountname="${item.bankAccountName ||
        ''}"  data-bankaccountnumber="${item.bankAccountNumber ||
        ''}" data-toggle="modal" data-isVerified="${item.isVerified || 'No'}" data-totalCars="${
        item.totalCars
      }"  data-avatar="${avt || item.avatar}"   data-id="${item._id}" data-totalCars="${
        item.totalCars
      }" data-firstname="${item.firstName || item.name}" data-lastname="${item.lastName ||
        item.name}" data-email="${item.email}" data-phone="${item.phoneNumber ||
        item.phone}" data-username="${item.userName || item.name}" data-address="${item.address ||
        'Enter your address'}" data-status="${item.status}" data-certificate="${item.businessName ||
        ''}"  class="table-action-btn"><i class="md md-edit"></i></a>
                    <a onclick="deleteRecord(this)" data-id="${
                      item._id
                    }" data-url="/partners"  class="table-action-btn "><i class="md md-close"></i></a></td>
                </tr>`;
  ...
  static runAdminSOS(datas, previledges) {
    ...
    data.map((item, i) => {
      ...
      //                    </td>
      template2 = `<tr>
                         <td class=""><a onclick="viewRecordTemplate(this)" href="#" id="plancat${
                           item._id
                         }" data-id="${
        item._id
      }" data-url="/admin-sos-detail" class=""><b>${formatDate(
        new Date(item.created_at),
      )} </b></a> </td>
                          ...
                          <td class="">CMT-USER- ${item.username}</td>
                           ...
                           <td class=""><span class="label ${className}">${item.status}</span></td>
                           <td class="">
                               <a onclick="viewRecordTemplate(this)" href="#" data-plate_number="${
                                 item.plate_number
                               }" data-date="${formatDate(
        new Date(item.created_at),
      )}" data-user_id="${
        item.user_id
      }"  data-media="https://Goom Logistics-bucket.s3.amazonaws.com/${
        item.media[0]
      }" data-location="${item.location}" data-status="${item.status}"  id="plancat${
        item._id
      }" data-id="${item._id}" data-username="${item.username}" data-email="${
        item.email
      }" data-phone_number="${
        item.phone_number
      }" data-url="/admin-sos-detail" class="table-action-btn"><i class="md md-edit"></i></a>
                               <a onclick="deleteRecord(this)" data-id="${
                                 item._id
                               }" data-url="/redflag-sos"  id="delete" class="table-action-btn "><i class="md md-close"></i></a></td>
      ...
      template2 = `<tr>
        ...
        new Date(item.created_at),
                          ...
                          <td class=""> ${item.email}</td>
                           ...
                           <td class=""><span class="label ${className}">${item.status}</span></td>
                           <td class="">
                               <a onclick="viewRecordTemplate(this)" href="#" data-assigned_to="${
                                 item.assigned_to
                               }" data-username="${item.username}" data-response="${
        item.response
      }" data-email="${item.email}" data-phone_number="${item.phone_number}" data-ticket_id="${
        item.ticket_id
      }" data-date="${formatDate(new Date(item.created_at))}" data-user_id="${
        item.user_id
      }" data-category="${item.category}"  data-comment="${item.comment}" data-subject="${
        item.subject
      }" data-status="${item.status}"  id="plancat${item._id}" data-id="${
        item._id
      }" data-url="/admin-ticket-detail" class="table-action-btn"><i class="md md-edit"></i></a>
                               <a onclick="deleteRecord(this)" data-id="${
                                 item._id
                               }" data-url="/tickets"  id="delete" class="table-action-btn "><i class="md md-close"></i></a></td>
  ...
  static runAdminFaqs(datas, previledges) {
    ...
    data.map((item, i) => {
      ...
      template2 = `<tr>
        ...
        item._id
      }" data-url="/admin-faq-detail" class=""><b>${formatDate(
        new Date(item.created_at),
      )} </b></a> </td>
                          ...
                          <td class=""> ${item.question}</td>
                           ...
                           <td class="">
                               <a onclick="viewRecordTemplate(this)" href="#" data-status="${
                                 item.status
                               }"  data-question="${item.question}"  data-answer="${
        item.answer
      }" id="plancat${item._id}" data-id="${
        item._id
      }" data-url="/admin-faq-detail" class="table-action-btn"><i class="md md-edit"></i></a>
                               <a onclick="deleteRecord(this)" data-id="${
                                 item._id
                               }" data-url="/faqs"  id="delete" class="table-action-btn "><i class="md md-close"></i></a></td>
  ...
  static runAdminCarsMgt(datas, carsInfo, drivers, partners, previledges) {
    ...
    partners.map((item, i) => {
      selectOptionsPart += `<option data-username="${item.name || item.firstname}" data-email="${
        item.email
      }" id="${item._id}" >${item.email}</option>`;
    });
    ...
    data.map((item, i) => {
      ...
      template2 = `<tr>
        ...
        new Date(item.created_at),
                          ...
                          <td class=""><span class="label ${className2}">${
        item.health_status
      }</span></td>

                          <td class=""><span class="label ${className3}">${
        item.car_status
      }</span></td>
                           <td><img  style="width:100px;height:100px" src="${item.imagePath ||
                             item.images}" /></td>
                           <td class="">
                               <a onclick="viewCarRecordTemplate(this)" data-tin="${
                                 item.vehicleIdentificationNumber
                               }" data-inspection_date="${
        item.inspectionDate
      }" data-inspection_time="${item.inspectionTime}" data-car_status="${
        item.car_status
      }" data-health_status="${item.health_status}" data-model_make_id="${item.model_make_id ||
        item.car.model_id}" data-trim="${item.car.model_trim}" data-old_car="${item.imagePath ||
        item.images}" href="#" data-model="${item.carModel ||
        item.car_type}"  data-car_type="${item.carModel || item.car_type}" data-car_id="${
        item._id
      }" data-assigned_driver_name="${item.assigned_driver_name}" data-assigned_driver_email="${
        item.assigned_driver_email
      }" data-checkmate="${item.assigned_driver_name}-${
        item.assigned_driver_email
      }" data-date="${formatDate(new Date(item.created_at))}"  data-partner_id="${item.creator ||
        'owned by company'}" data-model="${item.carModel ||
        item.model}"  data-car_year="${item.carYear ||
        item.car_year}" data-color="${item.vehicleColor || item.color}"  data-status="${
        item.status
      }" data-plate_number="${item.plateNo ||
        item.plate_number}" data-inspection_detail="${item.inspection_detail ||
        'No comment.'}" data-description="${item.carDescription || item.description}"  id="plancat${
        item._id
      }" data-id="${item._id}" data-license="${item.plateNo ||
        item.license}" data-url="/admin-car-mgt-detail" class="table-action-btn"><i class="md md-edit"></i></a>
                                <a onclick="deleteRecord(this)" data-id="${
                                  item._id
  ...
  static runAdminDriveTestAdd(url = 'add-drive-test', users, partners, previledges) {
    ...
    document.getElementById('savemesa').addEventListener('click', e => {
      ...
      if (email_x.options[email_x.selectedIndex].text == '--Select an email user--') {
        var notification = alertify.notify('Select an email user.', 'error', 5, function() {
          console.log('dismissed');
        });
      ...
      if (!date) {
        var notification = alertify.notify('Date required.', 'error', 5, function() {
          console.log('dismissed');
        });
      ...
      if (!car_id) {
        var notification = alertify.notify('car id/plate number required.', 'error', 5, function() {
          console.log('dismissed');
        });
      ...
      if (url != 'add-inspection') {
        if (!time) {
          var notification = alertify.notify('Time required.', 'error', 5, function() {
            console.log('dismissed');
          });
        ...
        if (!description) {
          var notification = alertify.notify(
            'Remarks/Comment about the car inspected is required.',
            'error',
            5,
            function() {
              console.log('dismissed');
      ...
      if (url != 'add-inspection') {
        ...
        updateUsersTestCenter(link, {
          test_center: me[0].test_center,
          test_center_address: me[0].test_center_address,
        });
      }
  ...
  static runAdminInspection(datas, carsInfo, drivers, partners, previledges) {
    ...
    data.map((item, i) => {
      ...
      template2 = `
                           ...
                           <div style=""  class="col-sm-6 col-lg-3 col-md-4 mobiles" >
                                    ...
                                    <div class="product-list-box thumb">
                                        <a href="#" class="image-popup" title="Screenshot-1">
                                            <img style="height:200px; " src="${item.imagePath ||
                                              item.images}" class="thumb-img" alt="work-thumbnail" />
                                        </a>

                                        <div class="product-action">
                                            <a href="#" class="btn btn-success btn-sm" onclick="viewInspectionUpdate(this);" href="#" data-contime="${
                                              item.confirmedInspectionTime
                                            }" data-condate="${
        item.confirmedInspectionDate
      }" data-id="${item._id}" data-inspection_date="${
        item.inspectionDate
      }" data-inspection_time="${item.inspectionTime}" data-car_status="${
        item.car_status
      }" data-health_status="${item.health_status}" data-model_make_id="${item.model_make_id ||
        ...
        item.license}" data-url="/admin-car-mgt-detail" class="table-action-btn"><i class="md md-mode-edit"></i></a>
                                <a href="#" class="btn btn-danger btn-sm"><i class="md md-close"></i></a>
                                        </div>


                                        <div class="detail">
                                            <h4 class="m-t-0"><a href="" class="text-dark">${
                                              item.car.car_name
                                            }</a> </h4>
                                            <div class="rating">
                                                <ul class="list-inline">
                                                    <li><span class="label ${className2}">${
        item.health_status
      }</span></li>
      ...
      template2 = `
                           ...
                           <div style=""  class="col-sm-6 col-lg-3 col-md-4 mobiles" >
                                    ...
                                    <div class="product-list-box thumb">
                                        ...
                                        <div class="product-action">
                                            <a href="#" class="btn btn-success btn-sm" onclick="viewRetrievalUpdate(this);" href="#" data-revoked="${
                                              item.hasBeenRevoked
                                            }" data-contime="${
        item.confirmedInspectionTime
      }" data-condate="${item.confirmedInspectionDate}" data-id="${
        item._id
      }" data-inspection_date="${item.inspectionDate}" data-inspection_time="${
        item.inspectionTime
      }" data-car_status="${item.car_status}" data-health_status="${
        item.health_status
      }" data-model_make_id="${item.model_make_id || item.car.model_id}" data-trim="${
        item.car.model_trim
      }" data-old_car="${item.imagePath || item.images}" href="#" data-model="${item.carModel ||
        item.car_type}"  data-car_type="${item.carModel || item.car_type}" data-car_id="${
        item._id
      }" data-assigned_driver_name="${item.assigned_driver_name}" data-assigned_driver_email="${
        item.assigned_driver_email
  ...
  <a style="display:none"  role="button" class="article__pointer article__pointer_blue" style="height: 30px; width:30px; margin-top:-100px;border:1px solid blue;" href="#article-blue">
    <span class="" role="presentation"><i class="ti-car" > </i></span>
      ...
      viewModals += `<div style="display:none" id="con-close-modal-${
        item._id
                                        ...
                                        <div class="m-t-20">
                                                    ...
                                                    <select disabled id="status${
                                                      item._id
                                                    }" class="selectpicker form-control" data-style="btn-white" tabindex="-98">
                                                       <option>--Select Status--</option>
                                                       <option>Booked</option>
                                        ...
                                        <div class="m-t-20">
                                                    <label for="position">Inspection Status</label>
                                                    <div>
                                                    <select disabled id="health_status${
                                                      item._id
                                                    }" class="selectpicker form-control" data-style="btn-white" tabindex="-98">
                                                        <option>--Select Status--</option>
                                                       <option>Pending</option>
                                    ...
                                    ${selectOptionsPart}
                                        ...
                                        <div class="m-t-20">
                                                    ...
                                                    <select disabled data-id="${
                                                      item._id
                                                    }" onchange="setCarDetail(this)"    id="car_model_make${
        item._id
                                        ...
                                        <div class="form-group">
                                            <label for="inputColor">Model Id</label>
                                            <input disabled id="car_model_id${
                                              item._id
                                            }" type="text" class="form-control"  >
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <label for="inputColor">Model Name</label>
                                            <input disabled id="car_model_name${
                                              item._id
                                            }" type="text" class="form-control" >
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <label for="inputColor">Trim Model</label>
                                            <input disabled id="car_model_trim${
                                              item._id
                                            }" type="text" class="form-control">
                                        </div>
                                        ...
                                        <div class="form-group ">
                                            <label for="inputColor">Plate Number</label>
                                            <input disabled id="plate_number${
                                              item._id
                                            }" type="text" class="form-control"  >
                                        </div>
                                      ...
                                      <div class="form-group" style="display:none">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea disabled id="description${
                                              item._id
                                            }" class="form-control autogrow" id="inputCarDescription" placeholder="The car is neat and the engine is working properly." style="overflow: hidden; word-wrap: break-word; resize: horizontal; height: 104px;"></textarea>
                                        </div>

                                        <br/>
                                        <div class="form-group" style="display:none">
                                            <label for="inputLicense">License Plate Number</label>
                                            <input disabled type="text" class="form-control" id="inputLicense${
                                              item._id
                                            }" >
                              ...
                              <div class="form-group">
                                  <label for="">Car Inspection Details</label>
                                  <textarea disabled id="inspection_detail${
                                    item._id
                                  }" type="text" class="form-control"  >Inspection detail</textarea>
                              </div>


                              <div class="form-group">
                                  <label for="">Car Inspection Date</label>
                                  <input  disabled id="inspection_date${
                                    item._id
                              ...
                              <div class="form-group">
                                  <label for="">Car Inspection Time</label>
                                  <input disabled id="inspection_time${
                                    item._id
                                  }" type="text" class="form-control" placeholder="HH:MM:AM/PM" >
                              </div>
                              ...
                              <div class="form-group" style="display:none">
                              <label class="control-label">Upload Image</label>
                              <input disabled onchange="initCarUpload(this)" data-id="${
                                item._id
                              }" type="file" class="filestyle" data-placeholder="No file" id="image-file${
        item._id
      }">
                              </div>


                              <img style="display:none" class="review-car" src="${
                                item.images
                              }"  id="img${
        item._id
      }" title="cars are here" data-carinfo="hello car"  style="width:100px;height:100px" /><span id="oldcar"></span>
                               ...
                               <br/>
                                                ...
                                                <div style="clear:both;display:table;margin-right:0px">
                                                <button style="display:none" style="margin-right:5px;display:none" onclick="addCarRecordEvent(this)" data-id="${
                                                  item._id
                                                }" data-url="/add-cars" id="create" style="display:none" type="button" class="btn btn-success waves-effect" data-dismiss="modal">Create</button>
                                                     <button style="display:none" style="margin-right:5px;" onclick="deleteData(this)" data-id="${
                                                       item._id
                                                     }" data-url="/faqs" id="delete" type="button" class="btn btn-danger waves-effect" data-dismiss="modal">Delete</button>
                                                    <button style="margin-right:5px;" id="cancle" data-id="${
                                                      item._id
                                                    ...
                                                    <button style="margin-right:5px;display:none" data-old_car="${
                                                      item.images
                                                    }" onclick="updateInspectionAction(this)" data-revoked="${
        item.hasBeenRevoked
      }" data-contime="${item.confirmedInspectionTime}" data-condate="${
        item.confirmedInspectionDate
      }" data-id="${item._id}" data-inspection_date="${
        item.inspectionDate
      }" data-inspection_time="${item.inspectionTime}" data-car_status="${
        ...
        item.license}" data-url="/admin-car-mgt-detail" data-url="/admin-inspection-detail" id="update" type="button" class="btn btn-info waves-effect waves-light">Revoke Car</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
  ...
  static runAdminCarRetrievalEdit(datas, carsInfo, drivers, partners, previledges) {
    ...
    data.map((item, i) => {
      ...
      template2 = `<tr>
                         <td class=""><a  href="#" id="plancat${item._id}" data-id="${
        item._id
      }" data-url="/" class=""><b>${formatDate(new Date(item.date_created))} </b></a> </td>
                          ...
                          <td class=""><span class="label ${className}">${item.status}</span></td>
                           <td class="">
                               <a onclick="showRetiveDetail(this)" id="${item._id}" data-id="${
        item._id
      }" data-name="${item.partnerName}" data-vin="${
        item.vehicleIdentificationNumber
      }" data-partnerid="${item.partner}" data-date="${item.date_created}" data-email="${
        item.partnerEmail
      }"  data-car="${item.vehicleName}" data-plate="${item.vehiclePlateNo}" data-carid="${
        item.vehicle
      }"  data-status="${item.status}" data-description="${
        item.retrievalComments
      }"  data-url="" class="table-action-btn"><i class="md md-edit"></i></a>
    ...
    itinerary.map((item, i) => {
      ...
      eachRecord = `
        ...
        item.status
                            ...
                            <td class="">
                               <a onclick="viewRecordItinsDetail(this)" href="#" data-checkmate="${
                                 item.assigned_driver_name
                               }-${item.assigned_driver_email}"  data-id="${
        item._id
      }" data-start_time="${formatDate(new Date(item.start_time))}" data-end_time="${formatDate(
        new Date(item.end_time),
      )}" data-id="${item._id}" data-no_hours="${item.no_hours}" data-start_location="${
        item.start_location
      }" data-destination="${item.destination}" data-drive_option="${
        item.drive_option
      }"  data-travel_option="${item.travel_option}"  data-username="${
        item.username
      }"  data-email="${item.email}"  data-phone_number="${item.phone_number}"  id="plancat${
        item._id
      }" data-assigned_driver_name="${item.assigned_driver_name}" data-status="${
        item.status
      }" data-id="${
        item._id
      }" data-url="/admin-itinerary-details" class="table-action-btn"><i class="md md-edit"></i></a>
  ...
  static runAdminPlans(usersPlan, previledges) {
    ...
    plans.map((item, i) => {
      ...
      template2 = `<tr>
                                        ...
                                        <td class=""><a onclick="getPlanId(this)" data-id="${
                                          item._id
                                        }" href="plan-detail">PLANID-${item.plan_id}</a></td>
                                            <td class="">   ${item.username} </td>

                                            <td class="">${item.email}</td>
                                            <td class="">${item.plan_name}</td>
                                            <td class="text-center "><span class="${className}">${
        item.status
      }</span></td>
                                            <td class="">  ${item.price}</td>
                                              ...
                                              <td class="">
                                                   <a id="plan-current-${
                                                     item._id
                                                   }" onclick="getBookingId(this)"  data-id="${
        item._id
      }" href="#" class="table-action-btn btn-custom btn-purple"><i class="md md-chevron-right"></i></a>
                                              </td>
  ...
  static runAdminPlansDetail(usersPlan, previledges) {
    ...
    setTimeout(() => {
        ...
        eachRecord = `
          ...
          item.status
                               ...
                               <td class="">${item.plan_id}</td>
                                <td class="">
                               <a onclick="viewRecordItinsDetail(this)" href="#" data-id="${
                                 item._id
          ...
          item._id
                               ...
                               <a onclick="deleteRecord(this)" data-id="${
                                 item._id
                               }" data-url="/itins-delete-from-plan"  id="delete" class="table-action-btn "><i class="md md-close"></i></a></td>
                           ...
                           <a href="#"><div class="col-sm-6  col-md-6 col-lg-4" >
                                <div class="widget-bg-color-icon card-box">
                                    ...
                                    <div class="col-lg-6 pull-right text-right">
                                        <h3 class="text-dark"><b className=""> ${item.carModel ||
                                          item.model} ${item.carYear || item.car_year}</b></h3>
      ...
      document.getElementById('send-quote').addEventListener('click', e => {
        ...
        if (status == '--choose--') {
          var notification = alertify.notify('Choose a status', 'error', 5, function() {
            console.log('dismissed');
          });
          return false;
        }
        ...
        if (amt && amt.length) {
          ...
          data_msg +=
            ...
            plan_ids +
            'on our platform  and the cumulated charges for the plan chosen is slated below.';
          data_msg += 'The total cost for this package including fare charges and other';
          data_msg +=
            ' related charges for this plan has been reviewed and slated at NGN ' + value_text;
          data_msg +=
            '. To proceed with this process, you are to make payments via the Goom Logistics platform. ';
          data_msg +=
            'If this message was sent to you via email you can login via the link below and proceed ';
          ...
          var notification = alertify.notify(
            'Error occured while processing update. Please ensure to update the fields on the quotation section ',
            'error',
            5,
            function() {
              console.log('dismissed');
    ...
    datas.map((item, i) => {
      ...
      eachRecord = `
                      <tr>
                          ...
                          <td class="">
                               <a onclick="viewRecordWalletTransactions(this)" data-url="/admin-wallet-detail" href="#" data-username="${
                                 item.full_name
                               }" data-email="${item.email}" data-phone_number="${
        item.phone_number
      }" data-reference="${item.reference}" data-date="${formatDate(
        new Date(item.created_at),
      )}" data-amount="${item.amount}" data-plan_id="${item.plan_id}"  data-quotation_id="${
        item.quotation_id
      }"  data-status="${item.status}"  id="plancat${item._id}" data-id="${
        item._id
      }"  class="table-action-btn"><i class="md md-edit"></i></a>
                               <a onclick="deleteRecord(this)" data-id="${
                                 item._id
                               }" data-url="/wallet"  id="delete" class="table-action-btn "><i class="md md-close"></i></a></td>
  ...
  static runAdminPlansManualBookings(dataCars, users, previledges) {
    ...
    $('#plan_name').on('change', function() {
      ...
      for (i = 0; i < lookup[selectValue].length; i++) {
        ...
        $('#plan_categories').append(
          "<option value='" + lookup[selectValue][i] + "'>" + lookup[selectValue][i] + '</option>',
        );
      }
    });
    ...
    let user_selected = [...new Set(users)].filter(
      item => item.email == emailsUser.options[emailsUser.selectedIndex].text,
    );
    ...
    document.body.addEventListener('click', function(e) {
      if (e.target.id == 'submitItinerary') {
        ...
        if (plan_name == '--Kindly select a plan --') {
          var notification = alertify.notify('Select a plan name', 'error', 5, function() {
            console.log('dismissed');
          });
        ...
        if (plancategory == '--Kindly select a category --') {
          var notification = alertify.notify(
            'Select a category for this plan',
            'error',
            5,
            function() {
              console.log('dismissed');
        ...
        if (car_1 == '--Kindly select a car --') {
          var notification = alertify.notify('Select a car for this plan', 'error', 5, function() {
            console.log('dismissed');
          });
        ...
        if (usr == '--Select user--') {
          var notification = alertify.notify('Select a username', 'error', 5, function() {
            console.log('dismissed');
          });
          return false;
        }
    ...
    document.getElementById('submitPlan').addEventListener('click', e => {
      ...
      if (ItineraryList.length <= 0) {
        var notification = alertify.notify(
          'You did not add any itinerary for this plan.',
          'error',
          5,
          function() {
            console.log('dismissed');
    ...
    datas.map((item, i) => {
      ...
      eachRecord = `
                      <tr>
                          ...
                          <td class="">
                               <a onclick="viewRecordWalletTransactions(this)"  data-url="/admin-payments-detail" href="#" data-username="${
                                 item.full_name
                               }" data-email="${item.email}" data-phone_number="${
        item.phone_number
      }" data-reference="${item.reference}" data-date="${formatDate(
        ...
        item._id
                               ...
                               <a onclick="deleteRecord(this)" data-id="${
                                 item._id
                               }" data-url="/payment"  id="delete" class="table-action-btn "><i class="md md-close"></i></a></td>
    ...
    datas.map((item, i) => {
      ...
      eachRecord = `
        ...
        item.status
                          ...
                          <td class="">
                               <a onclick="viewRecordWalletTransactions(this)" data-url="/admin-quotations-detail" href="#" data-username="${
                                 item.full_name
                               }" data-email="${item.email}" data-phone_number="${
        item.phone_number
      }" data-reference="${item.reference}" data-date="${formatDate(
        ...
        item._id
                               ...
                               <a onclick="deleteRecord(this)" data-id="${
                                 item._id
                               }" data-url="/quotation"  id="delete" class="table-action-btn "><i class="md md-close"></i></a></td>
    ...
    data.map((item, i) => {
      ...
      template2 = `<tr>
                          ...
                          <td >${item.username}</td>
                           ...
                           <td><span class="label ${className}">${item.status}</span></td>
                           <td >
                               <a onclick="viewDriveTest(this)"  data-id="${
                                 item._id
                               }" href="#" data-date="${item.createdDate}" data-username="${
        item.username
      }" data-description="${item.description}" data-phone_number="${
        item.phone_number
      }" data-email="${item.email}" data-date="${formatDate(
        new Date(item.createdDate),
      )}"    data-status="${item.status}" id="plancat${item._id}"    data-id="${
        item._id
      }" data-url="/admin-inspection-detail" class="table-action-btn"><i class="md md-edit"></i></a>
                                <a onclick="deleteRecord(this)" data-id="${
                                  item._id
                                }" data-url="/drive-test"  id="delete" class="table-action-btn "><i class="md md-close"></i></a></td>
    ...
    dataTrails.forEach(item => {
      ...
      eachRecord = `
         <tr>
            ...
            <td class=""><img  src="${
              item.avatar
            }"  style="height:30px;width30px;border-radius:50px"/></td>
  ...
  static runAdminRepairs(data) {
    ...
    items.forEach(item => {
      ...
      eachRecord = `
           <tr>
              ...
              <td class=""><span class="label ${className}">${item.status}</span></td>
              <td>
            <a onclick="viewRecordTemplate(this)" data-user_id="${item.user_id}" data-location="${
        item.location
      }" data-carbrand="${item.carbrand}" data-status="${item.status}" data-id="${
        item._id
      }"  data-url="/admin-mech-status"  id="delete" class="table-action-btn "><i class="glyphicon glyphicon-eye-open"></i></a>
    ...
    `;
      ...
      document.getElementById('carImage').src =
        'https://Goom Logistics-bucket.s3.amazonaws.com/' + image;
      // }
      recordItems.innerHTML += eachRecord;
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        var notification = alertify.notify('Could not perform upload. ', 'error', 5, function() {
          console.log('dismissed');
        });
      }
    }
