...
!(function(a) {
      ...
      return (
        ...
        (this.initializeModule = function(b) {
          ...
          (c.shouldInitialize = c.shouldInitialize || func.ok),
            c.shouldInitialize() &&
              (c.initialize && c.initialize(), c.events && dom.attachEvents(a, c.events));
        }),
        ...
        (this.removeModule = function(b) {
          var c = this.modules[b];
          c.shouldInitialize() &&
            (c.events && dom.detachEvents(a, c.events), c.destroy && c.destroy()),
  ...
  var Renderer = function(a, b, c, d) {
    ...
    dialog = renderer.create('<div class="modal" aria-hidden="false" tabindex="-1"/>', function(
      a,
      b,
    ) {
          ...
          (this.getPoints = function() {
            ...
            (this.scrollIntoView = function(a) {
              ...
              return (
                a.scrollTop + b < this.sc.offsetTop &&
                  (a.scrollTop += Math.abs(a.scrollTop + b - this.sc.offsetTop)),
                this
              );
            }),
            (this.normalize = function() {
              var a = function(a, b) {
                  if (
                    ...
                    (dom.isVisiblePoint(a) && dom.isBlock(a.node) && dom.isEmpty(a.node))
                  )
                    return a;
                  var c = dom.ancestor(a.node, dom.isBlock);
                  if (
                    ((dom.isLeftEdgePointOf(a, c) || dom.isVoid(dom.prevPoint(a).node)) && !b) ||
                    ((dom.isRightEdgePointOf(a, c) || dom.isVoid(dom.nextPoint(a).node)) && b)
                  ) {
            ...
            (this.splitText = function() {
              var b = a === e,
                g = this.getPoints();
              return (
                dom.isText(e) && !dom.isEdgePoint(this.getEndPoint()) && e.splitText(f),
                dom.isText(a) &&
                  !dom.isEdgePoint(this.getStartPoint()) &&
                  ((g.sc = a.splitText(d)), (g.so = 0), b && ((g.ec = g.sc), (g.eo = f - d))),
          ...
          (this.isOnEditable = h(dom.isEditable)),
            ...
            (this.wrapBodyInlineWithPara = function() {
              if (dom.isBodyContainer(a) && dom.isEmpty(a))
                return (a.innerHTML = dom.emptyPara), new c(a.firstChild, 0, a.firstChild, 0);
      ...
      return {
        ...
        createFromNode: function(a) {
          var b = a,
            ...
            e = dom.nodeLength(d);
          return (
            dom.isVoid(b) && ((c = dom.listPrev(b).length - 1), (b = b.parentNode)),
            dom.isBR(d)
    ...
    Editor = function(a) {
          ...
          .on('keydown', function(c) {
            c.keyCode === key.code.ENTER && a.triggerEvent('enter', c),
              ...
              c.isDefaultPrevented() ||
                (f.shortcuts ? b.handleKeyMap(c) : b.preventDefaultEditableShortCuts(c));
          })
          .on('keyup', function(b) {
      ...
      (this.tab = function() {
        ...
        (this.getSelectedText = function() {
          var a = this.createRange();
          return (
            a.isOnAnchor() && (a = range.createFromNode(dom.ancestor(a.sc, dom.isAnchor))),
            a.toString()
          );
        }),
    ...
    Clipboard = function(a) {
      ...
      (this.events = {
        ...
        (this.pasteByHook = function() {
          ...
          if (dom.isImg(b)) {
            for (
              var c = b.src, d = atob(c.split(',')[1]), e = new Uint8Array(d.length), f = 0;
              f < d.length;
              f++
            )
              e[f] = d.charCodeAt(f);
      ...
      (this.events = {
        ...
        (this.initialize = function() {
          (this.$handle = $(
            [
              ...
              '<div class="',
              e.disableResizeImage ? 'note-control-holder' : 'note-control-sizing',
              ' note-control-se"></div>',
              e.disableResizeImage ? '' : '<div class="note-control-selection-info"></div>',
              '</div>',
              '</div>',
            ].join(''),
          ).prependTo(d)),
        ...
        (this.update = function(b) {
          ...
          if ((a.invoke('imagePopover.update', b), c)) {
            ...
            d.css({ display: 'block', left: f.left, top: f.top, width: g.w, height: g.h }).data(
              'target',
              e,
            );
            var h = g.w + 'x' + g.h;
      ...
      (this.initialize = function() {
          ...
          return (
            b.fontInstalledMap.hasOwnProperty(a) ||
              (b.fontInstalledMap[a] =
                agent.isFontInstalled(a) || list.contains(e.fontNamesIgnoreCheck, a)),
            b.fontInstalledMap[a]
          );
        }),
            ...
            return c
              .buttonGroup([
                ...
                c.dropdown({
                  ...
                  template: function(a) {
                    'string' == typeof a &&
                      (a = { tag: a, title: f.style.hasOwnProperty(a) ? f.style[a] : a });
                    var b = a.tag,
                      c = a.title,
              ...
              return c
                .buttonGroup([
                  c.button({
                    className: 'dropdown-toggle',
                    contents:
                      '<span class="note-current-fontname"/> ' + c.icon(e.icons.caret, 'span'),
                    tooltip: f.font.name,
                    data: { toggle: 'dropdown' },
                  }),
                  c.dropdownCheck({
              ...
              return c
                .buttonGroup([
                  c.button({
                    className: 'dropdown-toggle',
                    contents:
                      '<span class="note-current-fontsize"/>' + c.icon(e.icons.caret, 'span'),
                    tooltip: f.font.size,
                    data: { toggle: 'dropdown' },
                  }),
                  c.dropdownCheck({
                  ...
                  children: [
                    ...
                    c.dropdown({
                      items: [
                        '<li>',
                        '<div class="btn-group">',
                        '  <div class="note-palette-title">' + f.color.background + '</div>',
                        '  <div>',
                      ...
                      callback: function(a) {
                        a.find('.note-holder').each(function() {
                          var a = $(this);
                          a.append(
                            c.palette({ colors: e.colors, eventName: a.data('event') }).render(),
                          );
                        });
                      },
                      click: function(b) {
            ...
            a.memo('button.paragraph', function() {
              return c
                .buttonGroup([
                  ...
                  c.dropdown([
                    c.buttonGroup({ className: 'note-align', children: [d, g, i, j] }),
        ...
        (this.tableMoveHandler = function(a) {
          ...
          h.css({ width: k.c + 'em', height: k.r + 'em' }),
            ...
            3 < k.c && k.c < e.insertTableMaxSize.col && i.css({ width: k.c + 1 + 'em' }),
            3 < k.r && k.r < e.insertTableMaxSize.row && i.css({ height: k.r + 1 + 'em' }),
            f.html(k.c + ' x ' + k.r);
        });
    },
    Toolbar = function(a) {
      ...
      (this.initialize = function() {
        ...
        if (e.maximumImageFileSize) {
          var g = Math.floor(Math.log(e.maximumImageFileSize) / Math.log(1024)),
            h =
              1 * (e.maximumImageFileSize / Math.pow(1024, g)).toFixed(2) + ' ' + ' KMGTP'[g] + 'B';
        ...
        var i =
          ...
          j =
            ...
            '</button>';
        this.$dialog = c
          .dialog({ title: f.image.insert, fade: e.dialogsFade, body: i, footer: j })
          .render()
          .appendTo(a);
      }),
        (this.destroy = function() {
      ...
      (this.initialize = function() {
        var a = e.dialogsInBody ? $(document.body) : d,
          ...
          g =
            ...
            '</button>';
        this.$dialog = c
          .dialog({ title: f.video.insert, fade: e.dialogsFade, body: b, footer: g })
          .render()
          .appendTo(a);
      }),
        (this.destroy = function() {
        ...
        return Object.keys(b)
          .map(function(c) {
            ...
            return (
              e
                .append(
                  $('<label><kbd>' + c + '</kdb></label>').css({ width: 180, 'margin-right': 10 }),
                )
    ...
    AirPopover = function(a) {
      ...
      (this.events = {
        ...
        'summernote.focusout': function(a, c) {
          agent.isFF ||
            (c.relatedTarget && dom.ancestor(c.relatedTarget, func.eq(b.$popover[0]))) ||
            b.hide();
        },
      }),
        (this.shouldInitialize = function() {
