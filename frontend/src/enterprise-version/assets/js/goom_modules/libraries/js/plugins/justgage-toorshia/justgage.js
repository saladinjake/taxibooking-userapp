...
JustGage = function(config) {
  ...
  if (obj.config.donut === true) {
    ...
    // title
    titleFontSize =
      widgetH / 8 > obj.config.titleMinFontSize ? widgetH / 10 : obj.config.titleMinFontSize;
    titleX = dx + widgetW / 2;
    titleY = dy + widgetH / 6.4;

    // value
    valueFontSize =
      widgetH / 6.5 > obj.config.valueMinFontSize ? widgetH / 6.5 : obj.config.valueMinFontSize;
    valueX = dx + widgetW / 2;
    valueY = dy + widgetH / 1.275;

    // label
    labelFontSize =
      widgetH / 16 > obj.config.labelMinFontSize ? widgetH / 16 : obj.config.labelMinFontSize;
    labelX = dx + widgetW / 2;
    labelY = valueY + valueFontSize / 2 + 5;

    // min
    minFontSize =
      widgetH / 16 > obj.config.minLabelMinFontSize ? widgetH / 16 : obj.config.minLabelMinFontSize;
    minX = dx + widgetW / 10 + ((widgetW / 6.666666666666667) * obj.config.gaugeWidthScale) / 2;
    minY = labelY;

    // max
    maxFontSize =
      widgetH / 16 > obj.config.maxLabelMinFontSize ? widgetH / 16 : obj.config.maxLabelMinFontSize;
    maxX =
      dx +
      widgetW -
      widgetW / 10 -
  ...
  if (obj.config.textRenderer) {
    ...
    obj.originalValue =
      humanFriendlyNumber(obj.originalValue, obj.config.humanFriendlyDecimal) + obj.config.symbol;
  } else if (obj.config.formatNumber) {
  ...
  if (obj.config.counter === true) {
    ...
    eve.on('raphael.anim.frame.' + obj.level.id, function() {
      ...
      if (obj.config.textRenderer) {
        ...
        obj.txtValue.attr(
          'text',
          humanFriendlyNumber(Math.floor(currentValue[0]), obj.config.humanFriendlyDecimal) +
            obj.config.symbol,
        );
      } else if (obj.config.formatNumber) {
...
JustGage.prototype.refresh = function(val, max) {
  ...
  if (obj.config.textRenderer) {
    ...
    displayVal =
      humanFriendlyNumber(displayVal, obj.config.humanFriendlyDecimal) + obj.config.symbol;
  } else if (obj.config.formatNumber) {
