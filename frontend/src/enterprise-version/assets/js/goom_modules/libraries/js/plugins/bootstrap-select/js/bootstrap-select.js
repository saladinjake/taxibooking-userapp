      ...
      createLi: function() {
        ...
        this.$element.find('option').each(function(index) {
          ...
          if (isOptgroup && $this.data('divider') !== true) {
            ...
            _li.push(
              generateLI(
                generateA(text, 'opt ' + optionClass + optGroupClass, inline, tokens),
                index,
                '',
                optID,
              ),
        ...
        if (this.options.size === 'auto') {
          var getSize = function() {
            ...
            $menu.css({
              ...
              overflow: 'hidden',
              'min-height':
                minHeight + headerHeight + searchHeight + actionsHeight + doneButtonHeight + 'px',
            });
            $menuInner.css({
              'max-height':
                menuHeight -
          ...
          $menu.css({
            'max-height':
              menuHeight + headerHeight + searchHeight + actionsHeight + doneButtonHeight + 'px',
            overflow: 'hidden',
            'min-height': '',
          });
          $menuInner.css({
          ...
          if (that.$searchbox.val()) {
            ...
            if (that.options.liveSearchNormalize) {
              $searchBase = $searchBase.not(
                ':a' + that._searchStyle() + '("' + normalizeToBase(that.$searchbox.val()) + '")',
              );
            } else {
              $searchBase = $searchBase.not(
        ...
        if (that.options.liveSearch) {
          ...
          if (!$this.val() && !/(38|40)/.test(e.keyCode.toString(10))) {
            if ($items.filter('.active').length === 0) {
              ...
              if (that.options.liveSearchNormalize) {
                $items = $items.filter(
                  ':a' + that._searchStyle() + '(' + normalizeToBase(keyCodeMap[e.keyCode]) + ')',
                );
              } else {
                $items = $items.filter(
