 ...
 window.addEventListener('DOMContentLoaded', () => {
  if (document.getElementById('sos-page')) {
     ...
     // $("#location-lat-long").val('Your browser doesn\'t support the geolocation api.');
        ...
        var notification = alertify.notify('Your browser does not support the geolocation api.', 'error', 5, function(){  console.log('dismissed'); });
      ...
      xhr.onreadystatechange = () => {
        if(xhr.readyState === 4){
          if(xhr.status === 200){
              ...
              var notification = alertify.notify('sos uploaded successfully', 'success', 5, function(){  console.log('dismissed'); });
              media_url = "https://Goom Logistics-bucket.s3.amazonaws.com/"+ file;

              var notification = alertify.notify('Processing please wait...', 'success', 15, function(){  console.log('dismissed'); });
            ...
            var notification = alertify.notify('Could not send sos response and upload unsuccessful.', 'error', 5, function(){  console.log('dismissed'); });
    ...
    */
    function getSignedRequest(file){
      ...
      xhr.open('GET', `https://demo-Goom Logistics-taxi-surf-api.herokuapp.com/api/v1/sign-s3?file-name=${file.name}&file-type=${file.type}`);
      ...
      xhr.onreadystatechange = () => {
        if(xhr.readyState === 4){
          if(xhr.status === 200){
            ...
            var notification = alertify.notify('Error uploading file to server.', 'error', 5, function(){  console.log('dismissed'); });
    ...
    */
    function initUpload(file){
      console.log( file)
      if(file == null){
        var notification = alertify.notify('No file was selected.', 'error', 5, function(){  console.log('dismissed'); });
