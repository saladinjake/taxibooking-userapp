      ...
      console.log(userPaymentDetails)
        ...
        var handler = PaystackPop.setup({
          ...
          callback: function(response){
             ...
             var notification = alertify.notify('Please wait while transaction is processing...', 'success', 25, function(){  console.log('dismissed'); });

                 setOldBalance(getBalance(),currentBalance) 

                 setTimeout(() =>{
                      updateAccountBalance(linkOfApi,record) //
           updateStatus(baseUrl+ "/"+  `plan-status/`+ userPaymentDetails.plan_id + '/update' , {status:'Paid', amount: userPaymentDetails.amount, has_updated: 'Yes' }) //itinerary status
           updateStatus(baseUrl+ "/"+  `itin-status/`+ userPaymentDetails.plan_id + '/update' , {status:'Ongoing'}) //plan status
           updateStatus(baseUrl+ "/"+  `quote-status/`+ userPaymentDetails.plan_id + '/update',{status:'Paid', amount: userPaymentDetails.amount}) //quotation
             getBalance()
  ...
  attachEvents(){
    ...
    if(document.getElementById("amount2")){
      ...
      PaymentWizard.initWizard()  
       var notification = alertify.notify('Updating account balance please wait...', 'success', 12, function(){  console.log('dismissed'); });
    ...
    static deductionOperation(oldBalance, newBalance){
      ...
      setTimeout(() =>{
       ...
       updateAccountBalance(linkOfApi,record) //
       updateStatus(baseUrl+ "/"+  `plan-status/`+ userPaymentDetails.plan_id + '/update' , {status:'Paid', amount: userPaymentDetails.amount, has_updated: 'Yes' }) //itinerary status
       updateStatus(baseUrl+ "/"+  `itin-status/`+ userPaymentDetails.plan_id + '/update' , {status:'Paid', has_received_payments:'Yes', has_received_quote:'Yes', user_plan_id:  userPaymentDetails.plan_id }) //plan status
       updateStatus(baseUrl+ "/"+  `quote-status/`+ userPaymentDetails.plan_id + '/update',{status:'Paid', amount: userPaymentDetails.amount, has_updated:'Yes'}) //quotation
       getBalance()
