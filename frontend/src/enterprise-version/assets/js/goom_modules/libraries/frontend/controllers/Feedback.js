        ...
        items.forEach(item => {
          ...
          const eachRecord = `
           <tr>
              ...
              <td class="">
              <a data-subject="${item.subject}" data-category="${item.category}" data-location="${
            item.location
          }" data-description="${
            item.comment
          }" data-points="-1" data-type="General Enquiries" class="table-action-btn read_more md-trigger" href="./record.html"  title=${
            item.reportType
          } class="comment" id=${item.id} onclick="function(e){e.preventDefault()};getId(this)"></a>
              </td>
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          //document.getElementById('preview').src = url;
          //document.getElementById('avatar-url').value = url;
          var notification = alertify.notify('Upload image Success.', 'success', 5, function() {
            console.log('dismissed');
          });
        } else {
          document.getElementById('filename-view').innerHTML = '';
          //alert('Could not upload file.');
          var notification = alertify.notify('Upload image error.', 'error', 5, function() {
            console.log('dismissed');
          });
        }
      }
